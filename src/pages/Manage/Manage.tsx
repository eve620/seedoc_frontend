...
export default () => {
    ...
    }, [isChangeFileSizeOpen])
    // 启动的时候加载用户信息
    const reload = async () => {
        const res = await getInstance().userListAll();
        console.log(res);
    ...
    return <div className="app-container">
        <Modal okText={"确定"} cancelText={"取消"} title={userForm?.title} open={isUserFormShow} onOk={userFormOk}
               onCancel={userFormCancel}>
            <form className="form">
                ...
                <div><span>授权目录</span></div>
                <Input size={"small"} type={"text"} value={userForm?.permission}
                       placeHolder={'为空表示可以访问所有目录'}
                       onChange={(permission) => setUserForm({...userForm!, permission})}/>
                <div><span>角色</span></div>
                <Select options={roles} onChange={(role) => setUserForm({...userForm!, role})}
                        value={userForm?.role}></Select>
            </form>
        </Modal>
        <Modal okText={"确定"} cancelText={"取消"} title="最大上传文件" open={isChangeFileSizeOpen}
               onOk={onChangeFileSizeOk} onCancel={onChangeFileSizeCancel}>
            <div onKeyDown={handleEnter}>
                <Input size={"small"} type={"text"} placeHolder={`请输入文件大小（MB），当前为${maxFileSize}MB`} ref={fileSizeInput}/>
            </div>
        </Modal>
